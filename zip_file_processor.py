...
class file_AnyFileToZip:
    DESCRIPTION = """将任意类型的文件或数据打包成 ZIP 文件\n\n支持两种打包方式：\n1. 路径打包：输入文件路径字符串或路径列表，将原始文件打包到 ZIP 中\n2. 内容打包：直接输入文件内容（如图像、文本等），会将内容转换后打包到 ZIP 中"""
    def __init__(self):
    ...
    def INPUT_TYPES(s):
        return {"optional": {"AnyFile_zip": (ANY_TYPE,),
                             "file_info": (NAME_INTO,),
                             "Outpath": ("STRING", {"multiline": False, "default": "", "placeholder": "解压文件夹路径"})},
    ...
    def _audio_to_wav_bytes(self, audio) -> bytes:
        ...
        with wave.open(buf, "wb") as wf:
            ...
            wf.writeframes(pcm)
        return buf.getvalue()
    def save_zip(self, AnyFile_zip=None, images=None, file_info=None, Outpath="", prompt=None, extra_pnginfo=None, **kwargs):
        ...
        # 解压到指定目录
        if Outpath and Outpath.strip():
            import shutil
            extract_dir = Outpath.strip().strip('"')
            # 确保目录存在
    ...
    def INPUT_TYPES(cls) -> dict:
        ...
        return {
            "required": {
                ...
                "custom_output_file": ("STRING", {"multiline": False, "default": "", "tooltip": "py|js|json （多个扩展名用 | 分隔）"}),
