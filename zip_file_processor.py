...
class file_LoadAnyFileList:
    ...
    def INPUT_TYPES(cls) -> dict:
        ...
        return {
            "required": {
                ...
                "max_files": ("INT", {"default": 0, "min": 0, "max": 10000, "step": 1})
            }
        }
    ...
    def load_file_paths(self, folder_path: str, custom_path: str, recursive: bool = True, output_file: str = "图像", custom_output_file: str = "", max_files: int = 0):
        extensions = []
        ...
        def collect_naming_info(file_path):
            # Collect info for files (already filtered by max_files since we're iterating over file_names_list)
            fname = os.path.basename(file_path)
            file_ext = os.path.splitext(fname)[1][1:].lower()
            if file_ext == "zip":
                return
        ...
        # Process files to respect max_files limit
        if is_file:
        ...
        # Clear naming_info and collect only for processed files
        naming_info = {}
        
        # Collect naming info for only the processed files (respect max_files)
        for i, fname in enumerate(file_names_list):
            # Use index to get the corresponding full path
            if i < len(full_paths_list):
                collect_naming_info(full_paths_list[i])
...
class file_save_ByList:
    ...
    def copy_rename_files(self, AnyFile, file_info, target_directory="", try_run=False):
        import shutil
        import json
        import os
        import torch
        ...
        # Handle case where parameters are passed as lists
        if isinstance(target_directory, list):
            if target_directory:
                # Join all elements if it's a list of path parts
                target_directory = ''.join(str(item) for item in target_directory)
            else:
                target_directory = ""
        ...
        # Clean up path - remove surrounding quotes and fix slashes
        target_directory = target_directory.strip()
        
        # Remove surrounding quotes - handle both single and double quotes
        # Also handle cases where quotes are escaped or only on one side
        while (target_directory.startswith('"') or target_directory.startswith("'")):
            target_directory = target_directory[1:].strip()
        while (target_directory.endswith('"') or target_directory.endswith("'")):
            target_directory = target_directory[:-1].strip()
        ...
        try:
            ...
            status = f"error: invalid directory path: {str(e)}"
            return (json.dumps(results), status)
        
        if not target_directory:
            status = "error: target_directory is required"
            return (json.dumps(results), status)
        ...
        if not os.path.exists(target_directory):
            if not try_run:
                try:
                    ...
                    status = f"error: failed to create directory: {str(e)}"
                    return (json.dumps(results), status)
            else:
                results.append(f"Would create directory: {target_directory}")
        ...
        AnyFile = _as_list(AnyFile)
        file_info = _as_list(file_info)
        
        for i, (content, info) in enumerate(zip(AnyFile, file_info)):
            try:
                # 获取原始文件名
                ...
                if isinstance(info, dict):
                    # 处理file_LoadAnyFileList的total_file_info输出
                    # 遍历所有扩展名，查找文件名
                    for ext, files in info.items():
                        if isinstance(files, list) and i < len(files):
                            filename = files[i]
                            break
                ...
                if not try_run:
                    if isinstance(content, str):
                        ...
                        with open(target_path, "w", encoding="utf-8") as f:
                            f.write(content)
                        results.append(f"Saved text file: {target_path}")
                    elif hasattr(content, "save"):
                        # 保存图像文件
                        content.save(target_path)
                        results.append(f"Saved image file: {target_path}")
                        ...
                        try:
                            ...
                            # 确保张量在CPU上
                            img_tensor = content.cpu()
                            
                            # 处理不同维度的张量
                            if img_tensor.ndim == 4:
                                img_tensor = img_tensor[0]  # 移除 batch 维度
                            ...
                            if img_np.shape[0] == 3:
                                ...
                                # 其他维度，尝试直接处理
                                results.append(f"Unsupported tensor shape: {img_np.shape}")
                                continue
                            
                            # 创建 PIL 图像并保存
                            img = Image.fromarray(img_np)
                            img.save(target_path)
                            results.append(f"Saved tensor image: {target_path}")
                        except Exception as e:
                            results.append(f"Error saving tensor image: {str(e)}")
                            # 打印详细错误信息
                            import traceback
                            results.append(f"Error details: {traceback.format_exc()}")
                    elif hasattr(content, "save_to"):
                        # 保存视频文件
                        try:
                            import io
                            buf = io.BytesIO()
                            ...
                            results.append(f"Saved video file: {target_path}")
                        except Exception as e:
                            results.append(f"Error saving video file: {str(e)}")
                    elif isinstance(content, dict) and "waveform" in content and "sample_rate" in content:
                        # 保存音频文件
                        try:
                            import wave
                            import io
                            ...
                            with open(target_path, 'wb') as f:
                                f.write(buf.getvalue())
                            results.append(f"Saved audio file: {target_path}")
                        except Exception as e:
                            results.append(f"Error saving audio file: {str(e)}")
                    elif isinstance(content, bytes):
                        # 保存字节数据
                        try:
                            with open(target_path, 'wb') as f:
                                f.write(content)
                            results.append(f"Saved binary file: {target_path}")
                        except Exception as e:
                            results.append(f"Error saving binary file: {str(e)}")
                    else:
                        # 尝试其他类型的保存
                        results.append(f"Unknown content type: {type(content)}")
                        results.append(f"Content repr: {repr(content)}")
                        results.append(f"Content dir: {[attr for attr in dir(content) if not attr.startswith('_')]}")
                        ...
                        try:
                            ...
                            results.append(f"Saved content as pickle: {target_path}")
                        except Exception as e:
                            results.append(f"Error saving as pickle: {str(e)}")
                else:
                    results.append(f"Would save file: {target_path}")
                    
            except Exception as e:
                results.append(f"Error processing file {i}: {str(e)}")
